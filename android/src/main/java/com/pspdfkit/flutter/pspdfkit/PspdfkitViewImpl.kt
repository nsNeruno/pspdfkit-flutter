...
class PspdfkitViewImpl : PspdfkitWidgetControllerApi {
    ...
    override fun canUndo(callback: (Result<Boolean>) -> Unit) {
        callback(
            Result.success(
                pdfUiFragment?.pdfFragment?.undoManager?.canUndo() == true
            )
        )
    }
    ...
    override fun undo(callback: (Result<Boolean>) -> Unit) {
        pdfUiFragment?.pdfFragment?.undoManager?.let {
            if (it.canUndo()) {
                it.undo()
                callback(Result.success(it.canUndo()))
                return
            }
    ...
    override fun canRedo(callback: (Result<Boolean>) -> Unit) {
        callback(
            Result.success(
                pdfUiFragment?.pdfFragment?.undoManager?.canRedo() == true
            )
        )
    }
    ...
    override fun redo(callback: (Result<Boolean>) -> Unit) {
        pdfUiFragment?.pdfFragment?.undoManager?.let {
            if (it.canRedo()) {
                it.undo()
                callback(Result.success(it.canRedo()))
                return
            }
            callback(Result.success(false))
        } ?: callback(Result.success(false))
    ...
    override fun getAnnotationState(callback: (Result<Map<String, Any?>?>) -> Unit) {
        ...
        callback(Result.success(mapOf("state" to state, "variant" to variant)))
    }
